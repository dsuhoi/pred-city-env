...
geojson_path = BASE_DIR.joinpath("geo_data.geojson")
with open(geojson_path, "r") as f:
    geojson = json.load(f)

select_city_vars = {"Все города": "all"} | {
    c["properties"]["title"]: i for i, c in enumerate(geojson["features"])
}
...
# }
select_metrics_vars = {
    "Индекс благополучия": "index_level",
...
def read_df_from_geojson(gj: dict) -> pd.DataFrame:
    ...
    for dist in gj["features"]:
        ...
        for key in ["title"] + list(select_metrics_vars.values()):
...
if current_city == "all":
    ...
    # ).json()
    geojson = geojson["features"][current_city]["districts"]
